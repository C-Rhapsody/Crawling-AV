...
list_url = {
    '야동.한국': [
        'https://nunagom.com/bbs/board.php?bo_table=torrentkor',
    ],
    '야동.유모': [
        'https://nunagom.com/bbs/board.php?bo_table=torrentcen',
        'https://nunagom.com/bbs/board.php?bo_table=torrentama&sca=MGS',
    ],
    '야동.노모': [
        'https://nunagom.com/bbs/board.php?bo_table=torrentama&sca=FC2',
        #'https://nunagom.com/bbs/board.php?bo_table=torrentuncen',
    ]
}
ua = UserAgent()
...
def main(dir, url):
    with requests.session() as s:
        ...
        while True:
            ...
            for table in tables:
                if (date.today() - timedelta(1)).strftime('%Y.%m.%d') == \
                table.xpath('./a/div/span[@class="pull-left"]/text()')[3].strip():
...
if __name__ == "__main__":
    for key, urls in list_url.items():
        for url in urls:
            main(key, url)